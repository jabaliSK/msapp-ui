...
import { RefreshCw, Server, CheckCircle, XCircle, Trash2, ShieldAlert, Save, MessageSquare, Check, AlertTriangle, Thermometer, Split, Layers } from 'lucide-react';

const ROLE_MAPPING = [
    { label: "LLM Generation", key: "llm" },
    { label: "Embedding", key: "embed" },
    { label: "Reranker", key: "rerank" },
    { label: "Guardrail", key: "guard" },
    { label: "OCR (Vision)", key: "ocr" },
    { label: "Decision Router", key: "router" } 
];
...
const ConfigPage = () => {
    ...
    try {
      ...
      if (currentConfigRes && currentConfigRes.data) {
          ...
          setConfig(prev => ({
              ...
              rerank: { port: s.rerank_port, model: s.rerank_model_id },
              guard: { port: s.guard_port, model: s.guard_model_id },
              ocr: { port: s.vl_port, model: s.vl_model_id },
              router: { port: s.router_port, model: s.router_model_id },
  ...
  const handleSaveConfig = async () => {
    ...
    const payload = {
        ...
        rerank_port: parseInt(config.rerank.port) || 8007,
        rerank_model_id: config.rerank.model || "Qwen/Qwen3-Reranker-0.6B",
        ...
        router_port: parseInt(config.router.port) || 8010, 
        router_model_id: config.router.model || "Qwen/Qwen3-0.6B", 
  ...
  return (
    // FIX: Added a wrapper div with h-full and overflow-y-auto to handle scrolling
    <div className="h-full overflow-y-auto w-full"> 
      <div className="p-8 max-w-5xl mx-auto relative">
              ...
              <div key={roleObj.key} className="p-6 flex items-center justify-between hover:bg-slate-50 transition-colors">
                <div className="w-1/3">
                  <h4 className="font-medium text-slate-800 flex items-center gap-2">
                      ...
                      {roleObj.key === 'rerank' && <Layers className="w-4 h-4 text-indigo-500" />}
                      {roleObj.label}
                  </h4>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden pb-10"> {/* Added pb-10 for bottom spacing */}
